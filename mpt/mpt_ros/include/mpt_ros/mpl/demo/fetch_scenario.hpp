...
namespace mpl::demo {
    ...
    public:
        FetchScenario(
            ...
            const std::shared_ptr<Mesh>& envMesh,
            const Frame& goal,
        ...
        template <class RNG>
        std::optional<State> sampleGoal(RNG& rng) const {
        ...
        std::pair<bool, S> isGoal(const State& q) const {
            // HACKY!  using ik solver to see if we're 0 steps away
            // from the goal!
            Robot robot(q);
            bool found = robot.ik(goal_, goalL_, goalEps_, 0);
            return { found, space_.distance(q, robot.config()) };
        }

        bool valid(const State& q, bool report = false) const {
            Robot robot(q);
            ...
            if (robot.inCollisionWith(environment_.get(), envFrame_, report))
                return false;
            
            return true;
        }

        bool link(const State& from, const State& to, bool report = false) const {
            using namespace unc::robotics::mpt;
            assert(valid(from, report));
            if (!valid(to, report)) {
            ...
            //     if (!valid(interpolate(from, to, i*delta)))
            //         return false;
            ...
            while (qStart != qEnd) {
                ...
                if (min == max) {
                    if (!valid(interpolate(space_, from, to, min * delta), report)) {
                        // if (report) JI_LOG(TRACE) << " @ t = " << min*delta;
                        return false;
                    }
                } else if (qEnd + 2 < qStart + queue.size()) {
                    std::size_t mid = (min + max) / 2;
                    if (!valid(interpolate(space_, from, to, mid * delta), report)) {
                    ...
                    for (std::size_t i=min ; i<=max ; ++i) {
                        if (!valid(interpolate(space_, from, to, i*delta), report)) {
